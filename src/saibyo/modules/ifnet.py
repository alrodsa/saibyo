...
class Head(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        """
        super().__init__()
        self._cnn0 = nn.Conv2d(3, 16, 3, 2, 1)
        self._cnn1 = nn.Conv2d(16, 16, 3, 1, 1)
        self._cnn2 = nn.Conv2d(16, 16, 3, 1, 1)
        self._cnn3 = nn.ConvTranspose2d(16, 4, 4, 2, 1)
        self._relu = nn.LeakyReLU(0.2, inplace=True)
...
class ResConv(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return self._relu(self._conv(x) * self._beta + x)
...
class IFBlock(nn.Module):
    ...
    def forward(
        ...
        feat = self._conv0(x)
        feat = self._conv_block(feat)

        tmp = self._last_conv(feat)
        tmp = F.interpolate(
...
class IFNet(nn.Module):
    ...
    def __init__(self, device: torch.device) -> None:
        ...
        """
        super().__init__()
        self._block0 = IFBlock(7+8, c=192)
        self._block1 = IFBlock(8+4+8+8, c=128)
        self._block2 = IFBlock(8+4+8+8, c=96)
        self._block3 = IFBlock(8+4+8+8, c=64)
        self._block4 = IFBlock(8+4+8+8, c=32)
        self._encode = Head()
        self._device = device
    ...
    def forward(
        ...
        f0 = self._encode(img0[:, :3])
        f1 = self._encode(img1[:, :3])
        ...
        flow = None
        mask = None
        block = [self._block0, self._block1, self._block2, self._block3, self._block4]
        ...
        for i in range(5):
            if flow is None:
                ...
                wf0 = warp(f0, flow[:, :2], self._device)
                wf1 = warp(f1, flow[:, 2:4], self._device)
                fd, m0, feat = block[i](
                    torch.cat(
                        (
                            warped_img0[:, :3],
            ...
            warped_img0 = warp(img0, flow[:, :2], self._device)
            warped_img1 = warp(img1, flow[:, 2:4], self._device)
